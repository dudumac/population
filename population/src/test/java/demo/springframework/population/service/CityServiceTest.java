...
public class CityServiceTest {
	...
	public void createCity() {
		...
		given(populationService.forCity(MALAGA)).willReturn(Optional.of(MALAGA_POPULATION));
		...
		then(actualCity.getId()).as("Check that City ID is set when stored.").isNotNull();
		then(actualCity).as("Check that City name is correct and city population is filled in.")
	...
	public void createCityWithIdThrowsException() {
		...
		then(throwable).as("An IAE should be thrown if a city with ID is passed")
				.isInstanceOf(IllegalArgumentException.class).as("Check that message contains the city name")
	...
	public void getCities() {
		...
		// let's say the service did not work for Amsterdam so it's stored without
		// population...
		final City amsterdam = new City(2L, AMSTERDAM, null);
		final City barcelona = new City(3L, BARCELONA, BARCELONA_POPULATION);
		given(cityRepository.getAllCities()).willReturn(Arrays.asList(malaga, amsterdam, barcelona));
		...
		SoftAssertions.assertSoftly(softly -> {
			softly.assertThat(cities).as("Should contain only two cities").hasSize(2);
			softly.assertThat(cities).extracting("population").as("Should not contain null populations")
					.doesNotContainNull();
			softly.assertThat(cities).extracting("name").as("Should contain names in alphabetical order")
